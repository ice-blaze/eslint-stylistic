...
export default createRule<MessageIds, RuleOptions>({
  ...
  create(context) {
    ...
    function normalizeOptions(options: any) {
      if (options && (options.FunctionDeclaration || options.FunctionExpression)) {
        let expressionOptions, patternOptions

        if (options.ArrayExpression)
          expressionOptions = normalizeOptionValue(options.FunctionExpression)

        if (options.ArrayPattern)
          patternOptions = normalizeOptionValue(options.FunctionDeclaration)

        return { FunctionExpression: expressionOptions, FunctionDeclaration: patternOptions }
      }

      const value = normalizeOptionValue(options as BasicConfig)

      return { FunctionExpression: value, FunctionDeclaration: value }
    }
     ...
     */
    function check(node: Tree.FunctionDeclaration | Tree.FunctionExpression): void {
      const elements = node.params
      ...
      const needsLinebreaks = (
        ...
        || (
          options.consistent
          && linebreaksCount > 0
          && linebreaksCount < node.params.length
        )
      )
